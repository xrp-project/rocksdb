...
print_block_handle(struct block_handle *handle) {
...
int main(int argc, char **argv) {
    ...
    if (fstat(sst_fd, &st) == -1) die("fstat() failed");

    offset = ((st.st_size - MAX_FOOTER_LEN) / EBPF_BLOCK_SIZE) *
             EBPF_BLOCK_SIZE;
    printf("footer offset (aligned to 512): %ld\n", offset);

    // data_buf = mmap(NULL, huge_page_size, PROT_READ | PROT_WRITE, MAP_HUGETLB
    // | MAP_HUGE_2MB | MAP_ANON | MAP_PRIVATE, -1, 0); if (data_buf ==
    // MAP_FAILED)
    //     die("mmap() failed");

    if (posix_memalign((void **)&data_buf, EBPF_DATA_BUFFER_SIZE,
                       EBPF_DATA_BUFFER_SIZE) != 0)
        die("data buf posix_memalign() failed");

    if (posix_memalign((void **)&scratch_buf, EBPF_SCRATCH_BUFFER_SIZE,
                       EBPF_DATA_BUFFER_SIZE) != 0)
        die("scratch buf posix_memalign() failed");
    sleep(2);
    ...
    for (int i = 0; i < 100; i++) {
        ...
        bpf_fd = -1234;
        long ret = syscall(SYS_READ_XRP, sst_fd, data_buf, 4096, offset, bpf_fd,
                           scratch_buf);

        printf("Return: %ld\n", ret);
        printf("%s\n", strerror(errno));

        if (ret < 0) die("read_xrp() failed");
