...
static uint64_t context_setup(int sst_fd, char *key, struct rocksdb_ebpf_ctx *ctx) {
    ...
    ctx->file_array[0].block_offset = block_offset;
    ctx->file_array[0].fd = sst_fd;
    ctx->file_array[0].stage = kFooterStage;
    ctx->file_array[0].offset = footer_offset;
    ctx->file_array[0].bytes_to_read = MAX_FOOTER_LEN;
